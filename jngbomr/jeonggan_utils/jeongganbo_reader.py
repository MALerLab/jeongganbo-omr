...
from ..inferencer import INFERENCER_DEFAULT_KWARGS
...
class JeongganboReader:
  def __init__(
    ...
    if self.run_omr:
      self.omr = Inferencer(**inferencer_kwargs)
  ...
  def _update_jeonggan_daegang(self):
    for gak in self.gaks:
      for i in range(len(gak.daegang_start_beats)):
        ...
        next_daegang_start_beat = (
          gak.daegang_start_beats[i+1] 
          if i+1 < len(gak.daegang_start_beats) 
          else len(gak)
        )
...
if __name__ == "__main__":
  ...
  split_page_dir = file_dir.parent.parent / 'dataset/jeongganbo_images/split_pages'
  
  image_path = 'haegeum_pg-031.png'
  image_path = 'daegeum_pg-059.png'
  image_path = 'gayageum_pg-271.png'

  reader = JeongganboReader()
  page = reader(split_page_dir / image_path)
  print(page)
  print(page.gaks)
